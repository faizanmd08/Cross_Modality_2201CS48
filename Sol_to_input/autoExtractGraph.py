...
def process_contracts(input_folder, output_folder):
    ...
    # Ensure required subdirectories exist for printResult (assuming './graph_data/node/' and './graph_data/edge/')
    os.makedirs('./graph_data/node/', exist_ok=True)
    os.makedirs('./graph_data/edge/', exist_ok=True)
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".sol"):
            ...
            # Generate node and edge features for the current file
            node_feature, edge_feature = generate_graph(filepath)
            node_feature = sorted(node_feature, key=lambda x: x[0])
            edge_feature = sorted(edge_feature, key=lambda x: (x[2], x[3]))

            # Output file path
            output_filename = f"{os.path.splitext(filename)[0]}_features.txt"
            ...
            with open(output_path, "w") as f:
                ...
                f.write(str(node_feature) + "\n\n")
                f.write("Edge Features:\n")
                f.write(str(edge_feature) + "\n")

            # Save results using printResult
            printResult(filename, node_feature, edge_feature)
...
if __name__ == "__main__":
    input_folder = "icse_Files"  # Folder with .sol files
    output_folder = "Graph_output"  # Folder to save processed results
    result_folder = process_contracts(input_folder, output_folder)
    print(f"Processed files saved in: {result_folder}")
