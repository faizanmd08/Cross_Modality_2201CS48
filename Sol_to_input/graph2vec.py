...
# Define your folders
node_folder = "graph_data/node/"
edge_folder = "graph_data/edge/"
...
def process_files():
    ...
    # Iterate through each file in the node folder
    for node_file in os.listdir(node_folder):
        node_path = os.path.join(node_folder, node_file)
        edge_path = os.path.join(edge_folder, node_file)  # Assuming node and edge files have the same name

        if os.path.exists(edge_path):  # Check if corresponding edge file exists
            # Process node file
            nodeNum, node_list, node_attribute_list = extract_node_features(node_path)
            ...
            results.append((node_file, file_result))
    
    return results

def save_results_to_folder(results, output_folder):
    for file_name, result_data in results:
...
if __name__ == "__main__":
    ...
    save_results_to_folder(results, output_folder)
