    ...
    def make_minibatch_iterator(self, data: Any, is_training: bool):
        ...
        while num_graphs < len(data):
            ...
            while num_graphs < len(data) and node_offset + len(data[num_graphs]['init']) < self.params['num_nodes']:
                ...
                # max_row_len = max([len(i) for i in cur_graph['init']])
                padded_features = [
                    np.pad(i, (0, 64 - len(i)), 'constant') for i in cur_graph['init']
                ]
                padded_features = np.pad(padded_features,
